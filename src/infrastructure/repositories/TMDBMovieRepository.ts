...
export class TMDBMovieRepository implements MovieRepository {
  ...
  async searchMovies(filters: MovieFilters): Promise<PaginatedResponse<Movie>> {
    ...
    return this.httpClient.get<PaginatedResponse<Movie>>('/search/movie', params)
  }

  async discoverMovies(filters: MovieFilters): Promise<PaginatedResponse<Movie>> {
    const params: Record<string, any> = {
      page: filters.page || 1,
    ...
    return this.httpClient.get<PaginatedResponse<Movie>>('/discover/movie', params)
  }
  ...
  async getGenres(): Promise<Genre[]> {
    const response = await this.httpClient.get<{ genres: Genre[] }>('/genre/movie/list')
    return response.genres
  }

  async getPopularMovies(page: number = 1): Promise<PaginatedResponse<Movie>> {
    return this.httpClient.get<PaginatedResponse<Movie>>('/movie/popular', { page })
  }
}
